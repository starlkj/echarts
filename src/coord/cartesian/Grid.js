...
*/
                ...
                if (labelUnionRect) {
                    ...
                    var labelAttr = 'height';
                    if( axis.isHorizontal() ) {
                        if( rotate >= 45 || 0 === rotate ) {
                            labelAttr = 'height';
                        } else {
                            labelAttr = 'width';
                        }
                    ...
                    var nameSize = titleShow ? 15 : 0;
                    var margin = labelShow ? axis.model.get('axisLabel.margin') : 0;
                    var labelSize = 0;
                    if (labelShow) {
                        if( rotate === 0 ) {
                            labelSize = labelUnionRect[labelAttr];
                        } else if( 45 === rotate ) {
                            labelSize = labelUnionRect[labelAttr] * 0.8;
                        } else {
                            labelSize = labelUnionRect[labelAttr] * 1.1;
                        }
                    }
                    // default setting
                    nameSize = labelSize + margin + nameSize;
                    gridRect[gridAttr] -= nameSize === 0 ? 5 : nameSize;
                    axis.model.mergeOption({'nameGap': labelShow ? labelSize + margin : margin});
                    axis.model.mergeOption({'labelSize': labelSize});

                    if (axis.position === 'top') {
                        gridRect.y += labelUnionRect.height + margin;
                    }
                    else if (axis.position === 'left')  {
                        gridRect.x += 0 === nameSize ? 5 : nameSize;
                        // gridRect.x += 30;   // 잘리는 상황 보정
                    }
                    // -- edit by eltriny - #20180128-02
