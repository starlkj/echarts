...
// 이전 라벨 옵션을 신규 라벨 옵션으로 변경 ( ver 3.x -> ver 4.x )
function convertNewLabelOpts( labelOpts ) {
    if( labelOpts.normal ) {
        // convert textStyle
        if( labelOpts.normal.textStyle ) {
            var textStyle = labelOpts.normal.textStyle;
            var styleKeys = Object.keys( textStyle );
            for( var idx = 0, nMax = styleKeys.length; idx < nMax; idx++ ) {
                labelOpts.normal[styleKeys[idx]] = textStyle[ styleKeys[idx] ];
            }
            delete labelOpts.normal.textStyle;
        }
        // convert normalStyle
        var normalStyle = labelOpts.normal;
        var styleKeys = Object.keys( normalStyle );
        for( var idx = 0, nMax = styleKeys.length; idx < nMax; idx++ ) {
            labelOpts[styleKeys[idx]] = normalStyle[ styleKeys[idx] ];
        }
        delete labelOpts.normal;
    }
    return labelOpts;
}   // function - convertNewLabelOpts
 ...
 */
echartsProto.setOption = function (opts, notMerge, lazyUpdate) {
    if (__DEV__) {
        assert(!this[IN_MAIN_PROCESS], '`setOption` should not be called during main process.');
    }

    // console.info( '>>>>>>>>>>>>>>>>>>>>>>>>>> eltriny <<<<<<<<<<<<<<<<<<<<<<' );
    // console.info( '>>>> eltriny - opts', opts );
    // console.info( '>>>> eltriny - opts', JSON.stringify( opts ) );

    // add by eltriny - #20180128-01 : - Start
    var option = zrUtil.clone(opts);
    if( option.label && option.label.normal ) {
        option.label = convertNewLabelOpts( option.label );
        for( var idx = 0, nMax = option.series.length; idx < nMax; idx++ ) {
            var objSeries = option.series[idx];
            if( objSeries.label ) {
                objSeries.label = convertNewLabelOpts( objSeries.label );
            } else {
                objSeries.label = {};
            }

            for (var key in option.label) {
                if (!objSeries.label.hasOwnProperty(key)) {
                    objSeries.label[key] = option.label[key];
                }
            }
        }
        delete option.label;
    }
    // add by eltriny - #20180128-01 : - End
 ...
 * add by starlkj - #20160629-01 : setShowDataZoom 추가
